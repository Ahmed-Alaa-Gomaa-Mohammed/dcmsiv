...
- **FR-007**: The tool MUST compute the SHA-1 hash of the raw, uncompressed pixel data layer:
  - For single-layer scans: SHA-1 hash of the single image frame raw pixel byte buffer.
...
- **MediaBase Record**:
  ...
  - `MediaHash`: Pre-calculated SHA-1 hexadecimal checksum representing the middle layer (for volumes) or single layer (for raster scans).
...
- SHA-1 hash is computed directly over the raw pixel byte buffer of the target frame, matching the hashing algorithm utilized by SIDEXIS when storing `MediaHash`.
